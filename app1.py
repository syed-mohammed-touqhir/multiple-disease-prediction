...
# User input
symptoms_input = st.text_input('Enter symptoms separated by commas (e.g., itching,skin_rash,joint_pain)')
...
if 'predicted_disease' in st.session_state:
    ...
    if 'show_info' in st.session_state:
        if st.session_state.show_info == 'Disease':
            ...
            st.markdown(f'<p style="font-weight: bold; font-size: 24px; color: #202020;">{st.session_state.predicted_disease}</p>', unsafe_allow_html=True)
        
        elif st.session_state.show_info == 'Description':
            desc, _, _, _, _ = helper(st.session_state.predicted_disease)
            st.subheader('Description')
            st.markdown(f'<p style="font-weight: bold; font-size: 24px; color: #202020;">{desc}</p>', unsafe_allow_html=True)
            ...
            for i, p in enumerate(pre, start=1):
                st.markdown(f'<p style="font-weight: bold; font-size: 24px; color: #202020;">{i}: {p}</p>', unsafe_allow_html=True)
            ...
            for i, m in enumerate(med, start=1):
                st.markdown(f'<p style="font-weight: bold; font-size: 24px; color: #202020;">{i}: {m}</p>', unsafe_allow_html=True)
            ...
            for i, w in enumerate(wrkout, start=1):
                st.markdown(f'<p style="font-weight: bold; font-size: 24px; color: #202020;">{i}: {w}</p>', unsafe_allow_html=True)
            ...
            for i, d in enumerate(die, start=1):
                st.markdown(f'<p style="font-weight: bold; font-size: 24px; color: #202020;">{i}: {d}</p>', unsafe_allow_html=True)
...
st.markdown("""
    ...
    body {
        background-color: #D9DBDE; /* White background */
        color: #202020; /* Black text */
    }
